// طباعة سند قيد يومية — تفتح نافذة جديدة بكود HTML أنيق (RTL + خط Cairo) ثم تستدعي الطباعة

import { AR_ENTRY_STATUS, AR_SOURCE, fmtDate, fmtDateTime } from '@/lib/format'
...
export function printJournalEntry(entry: PrintEntry): boolean {
  ...
  const subLine = systemSubLine(tpl, 'قيود اليومية')
  const rows = entry.lines
    .map(
      (l, i) => `
        <tr>
          <td class="num">${i + 1}</td>
          <td><span class="num code">${escapeHtml(l.account.code)}</span> ${escapeHtml(l.account.name)}</td>
          <td>${l.description ? escapeHtml(l.description) : '—'}</td>
          <td>${l.costCenterName ? escapeHtml(l.costCenterName) : '—'}</td>
...
<title>سند قيد يومية — ${escapeHtml(entry.number)}</title>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0 }
  ...
  /* الترويسة الموحدة: الشركة يميناً + الشعار 4× متمركز + رقم القيد ووقت الطباعة يساراً — على صف واحد */
  ...
  .tafqit-box { border: 1px solid #e5d3b3; border-radius: 6px; padding: 8px 12px; margin-top: 10px; background: #fffbeb; font-size: 12.5px }
  .tafqit-box .lbl { font-size: 10px; color: #92400e; font-weight: 700 }
...
<body>
  <div id="doc-root" data-company="${escapeHtml(company.companyName)}" data-doc-title="${escapeHtml(`سند قيد يومية ${entry.number}`)}" data-accent="#b45309">
  <div class="head">
    <div class="head-row">
      <div class="co">
      ...
      <div class="stamp-col">
        <div class="doc-no">
          ...
          <div class="val">${escapeHtml(entry.number)}</div>
        </div>
  ...
  <h2 class="doc-title"><span>سند قيد يومية</span></h2>

  <div class="meta">
    <div class="meta-item"><div class="lbl">التاريخ</div><div class="val num">${fmtDate(entry.date)}</div></div>
    <div class="meta-item"><div class="lbl">المصدر</div><div class="val">${AR_SOURCE[entry.source] ?? entry.source}</div></div>
    <div class="meta-item"><div class="lbl">الحالة</div><div class="val">${AR_ENTRY_STATUS[entry.status] ?? entry.status}</div></div>
    <div class="meta-item"><div class="lbl">عدد البنود</div><div class="val num">${entry.lines.length}</div></div>
  </div>

  <div class="desc"><div class="lbl">البيان</div>${escapeHtml(entry.description)}</div>

  <table data-print-main>
    <thead>
      <tr>
        <th style="width:34px">م</th>
        <th style="width:26%">الحساب</th>
        <th>البيان</th>
        <th style="width:14%">مركز التكلفة</th>
        <th class="num" style="width:14%">مدين</th>
        <th class="num" style="width:14%">دائن</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
    <tfoot>
  ...
  <div class="tafqit-box">
    <div class="lbl">تفقيط الإجمالي (مدين = دائن)</div>
    ${escapeHtml(tafqitSYP(entry.totalDebit))}
  </div>
